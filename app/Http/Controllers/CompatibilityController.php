 ...
 * - `/api/me/role` -> `/api/me/summaries` plus `/api/me/vendors` when the client needs richer user/vendor context.
...
class CompatibilityController extends Controller
{
     ...
     *
     * Prefer `/api/me/summaries` for a compact authenticated-user summary, `/api/vendor-applications/status`
     * for application workflow checks, or `/api/me/vendors` for the richer vendor payload.
     */
    #[Authenticated]
    #[Endpoint('Get seller status', 'Legacy compatibility alias for `/api/seller/status`. Prefer `/api/me/summaries` for a compact summary, `/api/vendor-applications/status` for application workflow checks, or `/api/me/vendors` for the richer vendor payload.')]
     ...
     *
     * Prefer `/api/me/summaries` for the canonical compact summary and `/api/me/vendors` when vendor-specific
     * state is needed. This legacy response collapses multiple concepts into one small payload.
     */
    #[Authenticated]
    #[Endpoint('Get user role summary', 'Legacy compatibility alias for `/api/me/role`. Prefer `/api/me/summaries` and `/api/me/vendors` for the canonical authenticated-user and vendor payloads.')]
