...
it('generates shared typescript contracts for the frontend app repeatedly in the same process', function (): void {
    ...
    expect(File::exists("{$path}/types.d.ts"))->toBeTrue()
        ...
        ->and(File::get("{$path}/types.d.ts"))->toContain('type UploadFilePart = {')
        ->and(File::get("{$path}/types.d.ts"))->toContain('readonly banner_image?: UploadFilePart')
        ->and(File::get("{$path}/types.d.ts"))->toContain('readonly images: UploadFilePart[]')
        ->and(File::get("{$path}/controllers/App/Http/Controllers/Public/index.ts"))->toContain('export type Request = Record<string, never>;')
        ->and(File::get("{$path}/controllers/App/Http/Controllers/Me/index.ts"))->toContain('export type Response = Spatie.LaravelData.PaginatedDataCollection<number, App.Data.ProductData>')
        ->and(File::get("{$path}/controllers/App/Http/Controllers/Me/index.ts"))->toContain('export type Response = App.Data.ShortVideoData')
        ->and(File::get("{$path}/controllers/App/Http/Controllers/Me/index.ts"))->toContain('export type Response = Spatie.LaravelData.PaginatedDataCollection<number, App.Data.NotificationData>')
        ->and(File::get("{$path}/controllers/App/Http/Controllers/Public/index.ts"))->toContain('export type Response = Array<App.Data.SectionData>')
        ->and(File::get("{$path}/controllers/App/Http/Controllers/Public/index.ts"))->toContain('export type Response = Array<App.Data.PaymentMethodData>')
        ->and(File::get("{$path}/controllers/App/Http/Controllers/Public/index.ts"))->toContain('export type Response = Array<App.Data.DeliveryOptionData>')
